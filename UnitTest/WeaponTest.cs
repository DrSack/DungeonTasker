...
namespace UnitTest
{
    public class WeaponTest
    {
        ...
        [Fact]
        public async Task TestWeaponEquipRealAsync()
        {
            string tempFile = Path.GetTempFileName();//create a temporary file
            File.WriteAllText(tempFile, "Weapons:IronDagger,IronBow,\nKeys:0\nGold:500\nEquipped:IronDagger\nItems:");
            WeaponInfoModel test = new WeaponInfoModel(new InventoryItemsModel(tempFile));

            Assert.True(await test.SetWeaponAsync(new ContentPage(), UserModel.CheckForstring(tempFile, "Equipped:")));
        ...
        [Fact]
        public async Task TestWeaponEquipNotEquippedAsync()
        {
            string tempFile = Path.GetTempFileName();//create a temporary file
            File.WriteAllText(tempFile, "Weapons:IronDagger,IronBow,\nKeys:0\nGold:500\nEquipped:IronDagger\nItems:");
            WeaponInfoModel test = new WeaponInfoModel(new InventoryItemsModel(tempFile));

            Assert.True(await test.SetWeaponAsync(new ContentPage(), "Not Equipped"));
        ...
        [Fact]
        public async Task TestWeaponEquipSameWeaponAsync()
        {
            string tempFile = Path.GetTempFileName();//create a temporary file
            File.WriteAllText(tempFile, "Weapons:IronDagger,IronBow,\nKeys:0\nGold:500\nEquipped:IronDagger\nItems:");
            WeaponInfoModel test = new WeaponInfoModel(new InventoryItemsModel(tempFile));
            await test.SetWeaponAsync(new ContentPage(), UserModel.CheckForstring(tempFile, "Equipped:"));
            Assert.False(await test.SetWeaponAsync(new ContentPage(), UserModel.CheckForstring(tempFile, "Equipped:")));//Test if false meaning same weapon cant be equipped
        }
